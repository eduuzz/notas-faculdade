...
import { X, Upload, FileText, AlertCircle, CheckCircle, Link2, ChevronDown, ChevronUp, Info, Sparkles } from 'lucide-react';
...
export default function ImportModal({ onClose, onImport, disciplinasExistentes = [] }) {
  const [modo, setModo] = useState('texto'); // 'texto', 'curriculo'
  ...
  // Parser inteligente para texto colado
  const parseTexto = useCallback((text) => {
    ...
    for (const linha of linhas) {
      ...
      if (!trimmed || trimmed.includes('Atividades Complementares')) continue;

      // Tentar extrair código
      const codigo = extrairCodigo(trimmed);
      
      if (codigo && CURRICULO_UNISINOS[codigo]) {
        // Encontrou no currículo conhecido
        ...
        // Formato esperado: "Código Nome 4 60" ou "Nome"
        ...
        if (partes.length >= 1) {
          ...
          if (codigo) {
            nome = trimmed.replace(codigo, '').trim();
          }

          // Tentar extrair créditos e carga horária
          const numeros = trimmed.match(/\d+/g);
          if (numeros && numeros.length >= 2) {
            const nums = numeros.map(n => parseInt(n));
            // Procurar por padrão típico: créditos (1-8) e carga horária (15-120)
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Cole o texto ou importe um currículo</p>
            </div>
          </div>
