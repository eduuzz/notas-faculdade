  ...
  const sidebarContent = (
    <>
      ...
      <div className="px-3 pt-4 pb-2">
        <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-md hover:bg-[var(--bg-hover)] transition-colors cursor-default">
          <img src="/icon-192.png" alt="Semestry" className="w-6 h-6 rounded-md flex-shrink-0" style={{ filter: 'var(--accent-icon-filter)' }} />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-[var(--text-primary)] truncate leading-tight">{primeiroNome}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 pb-3 space-y-0.5">
        <button onClick={toggleTheme} className="w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] transition-colors">
          {isDark ? <Sun size={16} className="flex-shrink-0" /> : <Moon size={16} className="flex-shrink-0" />}
          {isDark ? 'Modo Claro' : 'Modo Escuro'}
        </button>
        <button onClick={() => { onOpenSettings(); setMobileOpen(false); }} className="w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] transition-colors">
          <Settings size={16} className="flex-shrink-0" />
          Configurações
        </button>
        {onOpenAdmin && (
          <button onClick={() => { onOpenAdmin(); setMobileOpen(false); }} className="w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] transition-colors">
            <Shield size={16} className="flex-shrink-0" />
            Painel Admin
          </button>
        )}
        <div className="border-t border-[var(--border-card)] mt-1 pt-1">
          <button onClick={() => { onOpenLogout(); setMobileOpen(false); }} className="w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] text-red-400/70 hover:text-red-400 hover:bg-[var(--bg-hover)] transition-colors">
