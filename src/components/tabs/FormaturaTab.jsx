    ...
    <motion.div className="space-y-3" variants={staggerContainer} initial="initial" animate="animate">
      ...
      <motion.div variants={staggerItem}>
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center p-3 rounded-lg bg-[var(--accent-bg10)] border border-[var(--accent-ring)]">
            <div className="text-2xl font-bold mb-0.5" style={{ color: 'var(--accent-400)' }}>{previsaoFormatura}</div>
            <div className="text-[var(--text-muted)] text-[11px]">Conclusão Prevista</div>
          </div>
          <div className="text-center p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-card)]">
            <div className="text-2xl font-bold text-amber-400 mb-0.5">{calcularAnosAteFormar()}</div>
            <div className="text-[var(--text-muted)] text-[11px]">Anos até Formar</div>
          </div>
          <div className="text-center p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-card)]">
            <div className="text-2xl font-bold text-emerald-400 mb-0.5">{progressoTotal.toFixed(0)}%</div>
            <div className="text-[var(--text-muted)] text-[11px]">Progresso do Curso</div>
          </div>
        </div>
      </motion.div>

      <motion.div variants={staggerItem}><div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <GlassCard className="p-4" hover={false}>
          ...
          <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
            {planejamentoSemestres.map((sem, index) => {
              ...
              return (
                <div key={index} className={`flex items-center gap-2 p-2 rounded-md ${isAtual ? 'bg-blue-500/8 border border-blue-500/20' : 'bg-[var(--bg-input)] border border-[var(--border-input)]'}`}>
          ...
          <button onClick={adicionarSemestre} className="mt-2 w-full py-2 border border-dashed border-[var(--border-input)] rounded-md text-[var(--text-muted)] text-xs flex items-center justify-center gap-1.5 hover:border-[var(--accent-ring)] hover:text-[var(--accent-400)] transition-colors">
            <Plus size={14} />Adicionar Semestre
          </button>
        </GlassCard>
        ...
        <GlassCard className="p-4" hover={false}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border-input)]">
                  <th className="text-left py-1.5 px-2 text-[var(--text-muted)] text-xs font-medium">Semestre</th>
                  <th className="text-center py-1.5 px-2 text-[var(--text-muted)] text-xs font-medium">Qtd</th>
                  <th className="text-center py-1.5 px-2 text-[var(--text-muted)] text-xs font-medium">Acum.</th>
                  <th className="text-right py-1.5 px-2 text-[var(--text-muted)] text-xs font-medium">%</th>
                </tr>
              </thead>
              <tbody>
                <tr className="border-b border-[var(--border-card)]">
                  <td className="py-2 px-2 text-emerald-400 text-xs">Aprovadas</td>
                  <td className="text-center py-1.5 px-2 font-bold text-emerald-400">{disciplinasAprovadas}</td>
                  <td className="text-center py-1.5 px-2 text-[var(--text-muted)]">{disciplinasAprovadas}</td>
                  <td className="text-right py-1.5 px-2 text-[var(--text-muted)]">{totalDisciplinas > 0 ? ((disciplinasAprovadas / totalDisciplinas) * 100).toFixed(0) : 0}%</td>
                </tr>
                {planejamentoSemestres.map((sem, index) => {
                  ...
                  return (
                    <tr key={index} className={`border-b border-[var(--border-card)] ${isAtual ? 'bg-blue-500/5' : ''}`}>
                      ...
                      <td className="text-center py-1.5 px-2 font-bold">{sem.quantidade}</td>
                      <td className="text-center py-1.5 px-2 text-[var(--text-muted)]">{acumulado}</td>
                      <td className="text-right py-1.5 px-2 text-[var(--text-muted)]">{percentual.toFixed(0)}%</td>
                    </tr>
                  );
                })}
                <tr className="bg-[var(--bg-input)] font-bold">
                  <td className="py-1.5 px-2 text-xs" style={{ color: 'var(--accent-400)' }}>Total Planejado</td>
          ...
          {disciplinasFaltando > 0 && (
            <div className="mt-2 p-2 bg-amber-500/8 border border-amber-500/20 rounded-md flex items-center gap-2">
          ...
          {disciplinasFaltando < 0 && (
            <div className="mt-2 p-2 bg-red-500/8 border border-red-500/20 rounded-md flex items-center gap-2">
          ...
          {disciplinasFaltando === 0 && (
            <div className="mt-2 p-2 bg-emerald-500/8 border border-emerald-500/20 rounded-md flex items-center gap-2">
